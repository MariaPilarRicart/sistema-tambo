...
export async function getDashboardResumen(periodo: DashboardPeriodoInput = 'hoy', customRange: CustomDateRange = {}) {
  ...
  const [
    ...
    findUltimosEventosSanitarios(),
    findAgendaTasksForDashboard({ estado: EstadoTarea.PENDIENTE, fechaProgramada: { lt: todayStart } }),
    findAgendaTasksForDashboard({ estado: EstadoTarea.PENDIENTE, fechaProgramada: { gte: todayStart, lte: todayEnd } }),
    findAgendaTasksForDashboard({ estado: EstadoTarea.PENDIENTE, fechaProgramada: { gt: todayEnd, lte: nextSevenDaysEnd } }),
