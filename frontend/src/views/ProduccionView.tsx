  ...
  return (
    <div className="module-page production-page">
      <section className="settings-header production-header">
        <div>
          <h2>Producción</h2>
          <span>Registro diario por fecha y turno.</span>
        </div>
      </section>
              ...
              <tbody>
                {registros.map((registro) => (
                  <tr key={registro.id} className={!registro.activo ? 'stock-inactive-row' : undefined}>
                    ...
                    <td className="production-number-cell">{formatLiters(ordeneTotal(registro))}</td>
                ...
                {registros.length === 0 && (
                  <tr>
                    <td colSpan={8}>Sin ordeñes para los filtros seleccionados.</td>
                  </tr>
                )}
              </tbody>
            </table>
