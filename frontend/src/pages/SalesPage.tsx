    ...
    () =>
      form.detalles.reduce(
        (total, detalle) => {
          ...
          return { litros: total.litros + litros, importe: total.importe + litros * precio };
        },
        { litros: 0, importe: 0 },
      ),
    [form.detalles],
    ...
    <div className="settings-page">
      ...
      <section className="panel">
        ...
        <form className="user-form production-form" onSubmit={handleSubmit}>
          <label><span>Cliente</span><select value={form.clienteId} onChange={(event) => setForm({ ...form, clienteId: event.target.value })} required><option value="">Seleccionar cliente</option>{clientesActivos.map((cliente) => <option key={cliente.id} value={cliente.id}>{cliente.razonSocial} - {cliente.cuit}</option>)}</select></label>
          <label><span>Número de factura</span><input value={form.numeroFactura} onChange={(event) => setForm({ ...form, numeroFactura: event.target.value })} required /></label>
          <label><span>Fecha de venta</span><input type="date" value={form.fechaVenta} onChange={(event) => setForm({ ...form, fechaVenta: event.target.value })} required /></label>
          <label className="production-wide-field"><span>Observaciones</span><textarea rows={2} value={form.observaciones} onChange={(event) => setForm({ ...form, observaciones: event.target.value })} /></label>

          <div className="production-wide-field table-wrap">
            <table className="users-table">
              <thead><tr><th>Lote de leche</th><th>Litros disponibles</th><th>Litros vendidos</th><th>Precio unitario</th><th>Subtotal</th><th>Acciones</th></tr></thead>
              <tbody>
                {form.detalles.map((detalle, index) => {
                  const lote = lotesLeche.find((item) => String(item.id) === detalle.loteLecheId);
                  const subtotal = Number(detalle.litrosVendidos || 0) * Number(detalle.precioUnitario || 0);
                  return (
                    <tr key={`${index}-${detalle.loteLecheId || 'nuevo'}`}>
                      <td><select value={detalle.loteLecheId} onChange={(event) => updateDetail(index, { loteLecheId: event.target.value })} required><option value="">Seleccionar lote</option>{lotesVendibles.map((option) => <option key={option.id} value={option.id}>{loteLabel(option)}</option>)}</select></td>
                      <td>{lote ? formatLiters(lote.litrosDisponibles) : '-'}</td>
                      <td><input type="number" min="0.01" max={lote?.litrosDisponibles ?? undefined} step="0.01" value={detalle.litrosVendidos} onChange={(event) => updateDetail(index, { litrosVendidos: event.target.value })} required /></td>
                      <td><input type="number" min="0" step="0.01" value={detalle.precioUnitario} onChange={(event) => updateDetail(index, { precioUnitario: event.target.value })} required /></td>
          ...
          <div className="production-wide-field selected-animals-summary">
            <button type="button" className="secondary-button" onClick={() => setForm({ ...form, detalles: [...form.detalles, { ...emptyDetail }] })}><Plus size={16} />Agregar lote</button>
            <span className="lineage-chip">Total: {formatLiters(totals.litros)}</span>
            <span className="lineage-chip">Importe: {formatCurrency(totals.importe)}</span>
          </div>
          <button type="submit" className="primary-button production-wide-field" disabled={isSaving || clientesActivos.length === 0 || lotesVendibles.length === 0}><Save size={18} />Registrar venta</button>
        </form>
      </section>
      ...
      <section className="panel">
        ...
        <form className="filters-form events-filters production-filters" onSubmit={handleFilters}>
          <label className="filter-field"><span>Cliente</span><select value={filters.clienteId} onChange={(event) => setFilters({ ...filters, clienteId: event.target.value })}><option value="">Todos</option>{clientes.map((cliente) => <option key={cliente.id} value={cliente.id}>{cliente.razonSocial}</option>)}</select></label>
        ...
        {isLoading ? <p className="table-empty">Cargando ventas...</p> : (
          <div className="table-wrap">
            <table className="users-table production-history-table">
              <thead><tr><th>Fecha</th><th>Factura</th><th>Cliente</th><th>CUIT</th><th>Litros vendidos</th><th>Precio total</th><th>Usuario</th><th>Detalle</th></tr></thead>
              <tbody>
                {ventas.map((venta) => (
                  <tr key={venta.id}>
                ...
                {ventas.length === 0 && <tr><td colSpan={8}>Sin ventas registradas.</td></tr>}
              </tbody>
            </table>
          </div>
        )}
        ...
        <div className="modal-backdrop">
          <div className="modal-panel animal-form-modal">
            ...
            <div className="dashboard-kpi-grid production-stats-grid">
              ...
              <article className="dashboard-kpi-card dashboard-kpi-blue"><strong>Precio total</strong><h3>{formatCurrency(selectedVenta.precioTotal)}</h3></article>
              <article className="dashboard-kpi-card dashboard-kpi-indigo"><strong>Usuario</strong><h3>{selectedVenta.usuario?.nombre ?? '-'}</h3></article>
            </div>
            <div className="table-wrap feed-table-wrap">
              <table className="users-table">
                <thead><tr><th>Lote</th><th>Litros vendidos</th><th>Precio unitario</th><th>Subtotal</th></tr></thead>
                <tbody>
